node(params.NodeSelector) {
    ...
    docker.image('notebooks:test').inside(' -v /etc/localtime:/etc/localtime:ro') {
        stage('Clone') {
            println('========================================== CLONE STAGE ==========================================')
        ...
        stage('Copy CT files') {
            ...
            dir("$env.WORKSPACE/openvino_notebooks") {
                sh 'curl -O https://storage.openvinotoolkit.org/data/test_data/openvino_notebooks/kits19/case_00030.zip'
        ...
        stage('Install dependencies') {
            println('======================================== PREPARE STAGE ==========================================')
            dir("$env.WORKSPACE/openvino_notebooks") {
                sh label: 'Upgrade pip', script: '. openvino_env/bin/activate && python -m pip install --upgrade pip'
